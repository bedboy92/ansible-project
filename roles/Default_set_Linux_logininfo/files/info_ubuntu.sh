...
Ipaddr=` ifconfig |grep "inet " |awk -F" " '{print $2}'|grep -v "127.0.0.1"`
PTotmem=`free -m | grep Mem | awk '{print $2}'`
PUsemem=`free -m | grep Mem | awk '{print $3}'`
PMemper=`expr \( ${PUsemem} \* 100 \/ ${PTotmem} \)`
STotmem=`free -m | grep 스왑 | awk '{print $2}'`
SUsemem=`free -m | grep 스왑 | awk '{print $3}'`
...
Chkfsys=`cat /home/gabia/diskuse | sort -r | uniq`
echo "                                                           "
